...
ggplot(D[!is.na(D$P),],aes(y=P,x=SSB)) + geom_point() + theme_classic() + labs(x="SSB (kt)", y="Surplus Production (kt)") + expand_limits(y=0,x=50) +   geom_hline(yintercept=0)   
...
#Plot stock status indicator based on SSB (examples below)
 ...
 mylrp <- (0.5*calc_last_10_years$SSBmsy) # 50% equilibirum SSBmsy using data from last 10 years
